...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const bobWithdrawAmount = utils.parseEther('0.08')
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // alice withdraws all fund via L1
    const aliceWithdrawAmount = utils.parseEther('0.05')
    const recipient = '0xDeaD00000000000000000000000000000000BEEf'
    const aliceChangeUtxoWithdrawal = new Utxo({
    ...
    await transaction({
      ...
      outputs: [aliceChangeUtxoWithdrawal],
      recipient: recipient,
      isL1Withdrawal: true,
    })
  ...
  it.only('should deposit from L1 and withdraw to L1', async function () {
    const { tornadoPool, token, omniBridge } = await loadFixture(fixture)
    const aliceKeypair = new Keypair() // contains private and public keys
    ...
    // transfer tx to Bob in a shielded pool
    // Bob gives Alice address to send some eth inside the shielded pool
    const bobKeypair = new Keypair() // contains private and public keys
    const bobAddress = bobKeypair.address() // contains only public key
    ...
    const bobSendUtxo = new Utxo({ amount: bobSendAmount, keypair: Keypair.fromString(bobAddress) })
    const aliceChangeUtxoSend = new Utxo({
      amount: aliceDepositAmount.sub(bobSendAmount),
      keypair: aliceDepositUtxo.keypair,
    })
    await transaction({
      tornadoPool,
      inputs: [aliceDepositUtxo],
      outputs: [bobSendUtxo, aliceChangeUtxoSend],
    })
    ...
    // Bob withdraws a part of his funds from the shielded pool
    const bobWithdrawAmount = utils.parseEther('0.05')
    const bobEthAddress = '0xEA674fdDe714fd979de3EdF0F56AA9716B898ec8'
    const bobChangeUtxo = new Utxo({ amount: bobSendAmount.sub(bobWithdrawAmount), keypair: bobKeypair })
    await transaction({
      tornadoPool,
      inputs: [bobReceiveUtxo],
    ...
    // withdraws all remaining fund from the shielded pool to L1
    const aliceWithdrawAmount = aliceDepositAmount.sub(bobSendAmount)
    const recipient = '0xDeaD00000000000000000000000000000000BEEf'
    const aliceChangeUtxoWithdraw = new Utxo({
      amount: aliceWithdrawAmount.sub(aliceWithdrawAmount),
      keypair: aliceKeypair,
    })
    await transaction({
      tornadoPool,
      inputs: [aliceChangeUtxoSend],
      outputs: [aliceChangeUtxoWithdraw],
      recipient: recipient,
      isL1Withdrawal: true,
    })
